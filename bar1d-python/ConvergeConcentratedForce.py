...
print("    Concentrated force in an element    : ||e||_L2 = %g h^%g" %(C, a))
a, C = np.polyfit(np.log(h_node),np.log(L2Norm_node),1)
print("    Concentrated force on a node        : ||e||_L2 = %g h^%g" %(C, a))
a, C = np.polyfit(np.log(h_even),np.log(EnNorm_even),1)
print("    Concentrated force in an element    : ||e||_en = %g h^%g" %(C, a))
